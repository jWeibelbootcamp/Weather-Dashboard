...
function getForecast(lat, lon) {
    ...
    fetch(queryURL).then(function (response) {
        ...
        for (var i = 0; i < highArray.length; i++) {
            ...
            var cardHeader = $("<div>").addClass("card-header");
            var cardBody = $("<div>").addClass("card-body");
            var imgEl = $("<img>").attr("src", `http://openweathermap.org/img/wn/${data.weather[0].icon}@.png`);// <-- something wrong
            ...
            $('#5-day-forecast').append(card.append(cardHeader.append(cardTitle.append(imgEl), cardBody.append(descriptionEl, tempEl, humidityEl))));
        }
    })
}
